...
- **增量渲染** — 智能检测 Markdown 内容变化，仅重新渲染变更部分，提升性能
- **渲染状态持久化** — 将渲染状态保存到 PPTX 文件，关闭 PowerPoint 后重新打开仍可增量更新
...
插件采用智能增量渲染机制，大幅提升重复渲染的性能和稳定性：

**核心特性：**
- **差异检测** — 使用 djb2 哈希算法计算每个元素的指纹，精确识别内容变化
- **部分重绘** — 仅重新渲染发生变化的元素，保持未变化部分不动
- **智能判断** — 当配置选项变化时自动触发完全重绘，确保渲染一致性
...
渲染状态（包括元素指纹、位置信息等）现在持久化存储在 PPTX 文档中：

**优势：**
- 关闭 PowerPoint 后重新打开，仍可进行增量渲染
- 避免因状态丢失导致的内容误删问题
- 跨会话保持渲染上下文

**实现方式：**
- 使用 Office 文档设置 API 存储序列化的 JSON 数据
- 内存缓存 + PPTX 持久化双层存储策略
- 版本控制机制确保数据兼容性
...
│   │   ├── diff-engine.test.ts   # 增量渲染差异检测单元测试
│   │   └── source-store.ts       # 渲染状态和源内容存储
│   ├── fonts/                    # 字体管理
...
**新增功能：**
- ✨ **增量渲染机制** — 智能检测 Markdown 内容变化，仅重新渲染变更部分，大幅提升渲染性能
- ✨ **渲染状态持久化** — 将渲染状态保存到 PPTX 文件，关闭 PowerPoint 后重新打开仍可增量更新
- ✨ **单元测试覆盖** — 为增量渲染差异检测添加 53 个测试用例，确保核心功能稳定性

**重要修复：**
- 🐛 **修复内容随机删除问题** — 彻底解决重复渲染时现有内容被意外删除的严重缺陷
  - 根本原因：RenderState 仅存储在内存中，关闭 PowerPoint 后丢失
  - 解决方案：将 RenderState 持久化到 PPTX 文件，采用内存缓存 + 持久化双层存储策略
- 🐛 **修复形状命名不稳定问题** — 通过形状数量差异精确识别新创建的形状，避免误命名非插件创建的形状

**改进：**
- 🎨 优化增量渲染差异检测算法，使用 djb2 哈希计算元素指纹
- 🎨 添加渲染状态版本控制，确保向后兼容性
- 🎨 增强形状管理逻辑，提升渲染稳定性
